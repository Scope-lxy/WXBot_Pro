...
- 私聊图片、图片+文字和多张图片都进入连续消息合并队列。私聊连续消息合并强制开启，普通文本静默等待为 `chat_message_merge_delay`，范围为 1-60 秒，默认 20 秒；设置为 1 秒时接近即时回复，但仍走同一条聚合链路。含图批次静默等待为基础值 `* 2` 且最多 60 秒；每轮最长等待为基础值 `* 3` 且最多 180 秒，默认 60 秒；单批最多合并 `9` 张图片。私聊 pending visual context 是 600 秒短期桥接，不靠关键词判断；成功发送图片相关回复后清理，之后围绕同一图片继续聊主要依赖聊天记录 history。
