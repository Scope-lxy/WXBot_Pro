...
保存聊天历史且对应的 `chat_context_repair_switch` / `group_context_repair_switch` 开启时，每个会话只在进程启动后的首次 AI 回复或监听窗口确认恢复后的首次 AI 回复读取一次当前窗口 `GetAllMessage()`；失败后固定退避再试，不周期巡检，也不滚动深历史。两个开关分别控制私聊 / 群聊 UI 补洞，`memory_context_switch` 只控制 history 是否带入 AI。监听恢复成功必须按 `(chat_type, conversation)` 重新标记待补洞，不能依赖消息正文或本地尾部检查推断恢复状态。
