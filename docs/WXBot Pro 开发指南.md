...
- 语音消息处理依赖微信自己的「语音自动转文字」结果，运行时不要调用 `msg.to_text()` 主动右键识别；私聊只有时长、暂无文字时每 5 秒重读一次当前可见消息，最多等待 20 秒，群聊不做延后重读。只有识别出正文的语音可以写入 memory 和进入 AI；未识别占位、`voicemsg` XML 和失败状态不进入 AI/history。
- 私聊失败语音不立即发送兜底，而是等私聊连续消息合并窗口结束后再判断；如果同批已有文本、成功识别语音或正在处理的有效私聊批次，就取消或跳过失败兜底。多条待识别语音里只要恢复出任意有效正文，超时或失败的其它语音只丢弃，不额外提示。
