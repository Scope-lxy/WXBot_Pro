...
- 全局监听正常路径只提前保存图片和引用消息，不提前保存普通文字/语音；轻量延后监听最终失败或过期时，才兜底保存暂存的普通文字和已识别语音，避免普通消息双写。
...
- 当前自动回复前优先走低风险补洞：如果 `venv/tools/wechat-cli/` 下的本地只读工具已初始化并通过当前账号校验，先从本地微信数据库读取最近上下文；读取条数由 `memory_context_count` 归一到 `50 ~ 200` 后加 `10` 条锚点缓冲，即 `60 ~ 210`。同一会话成功补洞后 `300` 秒内默认不重复补；CLI 最终失败不按时间冷却，而是记录连续失败次数，第一次失败跳过本轮补洞，第二次连续失败才读取当前微信窗口已渲染消息兜底。低风险 UI 读取内部上限为 `30`，私聊和群聊共用这套门禁；没有启动 / 恢复 / 尾部不匹配这类强原因时，成功 TTL 到期会以 `scheduled_low_risk_check` 做一次计划性低风险巡检。
...
- 补洞日志要克制：成功 TTL / 高风险冷却中、微信操作锁繁忙属于正常让路，不写日志；CLI 最终失败写 `WARNING` 并记录连续失败次数，第二次连续失败才尝试低风险 UI 兜底，不缓存“永久不可用”；低风险 `GetAllMessage()` 超过 `20` 秒仍保留 `WARNING` 慢操作告警，方便观察微信 UI 卡顿；真正失败和高风险读取异常继续告警。
- 通讯录维护分两条独立链路：手动 `立即建档` 优先使用 `wechat-cli contacts` 提取最多 `10000` 个可读好友的基础身份，CLI 不可用时回退 wxautox4 分批读取；自动维护仍固定走微信界面分批读取完整资料。CLI 基础身份自动同步不绑定 UI 自动维护，会在本地索引为空的启动后检查触发，并按面板 `contact_directory_auto_maintenance_full_scan_interval_days` 每 X 天跑一轮；失败后冷却 30 分钟再试。CLI 同步只更新昵称、备注、wxid 等基础身份，不写 `last_full_scan_completed_at`，也不清空 UI 才能拿到的地区、来源、添加时间、标签、个性签名等资料。发送、改备注、打标签等写操作仍只能走 wxautox4。
