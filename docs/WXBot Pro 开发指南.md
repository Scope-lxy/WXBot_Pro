...
动态监听采用按需补窗。普通增删监听不触发微信客户端重绑，不恢复主循环高频巡检。普通补窗失败先按 30s / 60s 延后，之后按 60s 继续尝试；持续 600s 仍不可用时标记降级并改为每 300s 低频恢复。`ListenerWindowSupervisor` 只保存会话名、重试时间和错误，不持有消息；消息在尝试补窗前已经进入 SQLite 事实库。同一会话只保留一项窗口恢复任务。只有“已监听但无子窗口”残留状态允许受控关闭重建。

固定监听可低频巡检补回，但不能和全局监听延后补窗抢微信 UI。

### 上下文补洞

保存聊天历史且 `memory_context_repair_switch` 开启时，AI 回复前只在启动首次回复、窗口恢复或当前入站不在本地尾部等强理由下读取一次当前窗口 `GetAllMessage()`，不周期巡检，也不滚动深历史；该开关只控制 UI 补洞，`memory_context_switch` 只控制已有 history 是否带入 AI，两者不得重新耦合。实机 wxautox4 的普通气泡 `Message.time` 为 `null`，时间只存在于独立 `TimeMessage`，因此快照规范化时让后续气泡继承最近的时间分隔，并保持同一时间组的窗口顺序。当前入站的实际接收时间是本次补洞硬上限；位于当前消息之前却被 wxautox4 解析成当天未来时刻的连续前缀只允许回退一天，仍无法满足窗口顺序时整批拒绝写入。

当前可见窗口只有在能与本地可信历史找到明确锚点时，才允许补入缺少的对方消息和人工 self 消息；无锚点快照既不落盘，也不进入本轮模型上下文。实时回调可把 `message.id/hash/hash_text` 作为原始观察事实保存，但只有 native ID 用于强去重，hash 和正文不能删除两次真实出现的同文消息，也不作为跨重启身份。私聊按方向、类型、内容和邻近时间逐次匹配，群聊额外包含发送者；语音转写在邻近时间匹配时忽略空白和标点差异，但不忽略时长壳差异；每条本地记录最多抵消一个可见气泡，因此重复短句和相同图片按出现次数保留。微信明确给出的时间分隔可用于邻近时间匹配；可见前缀或无时间快照为保持顺序而推算出的时间只决定插入位置，必须再按方向、类型和内容一对一抵消本地已有气泡，其中无时间短句不能被远距离历史同文直接抵消。推算值只作为内部 `received_at` 排序键，`native_time` 留空，并在存储元数据中保留 `time_inferred=true` 供跨重启排重；面板和模型 history 不得显示该排序值或内部标记。日期解析同时接受微信快照的 `YYYY-MM-DD` 与本地 `YYYY/MM/DD`，对外历史统一渲染为后者。快照比较、补差额和写盘必须在同一把会话记忆锁内完成，重复执行同一快照新增数必须为 `0`；补洞事件 ID 必须稳定，SQLite 唯一键负责收敛并发写入。补洞前读取该会话最近一次聊天记录删除边界，推算时间、无时间或不晚于该边界的候选不得重新写回。系统消息、`未播放` 和仅时长语音不写入模型上下文。冷却、锚点数量和读取上限均为内部参数，面板只暴露补洞总开关。
...
维护状态 `running` 只能阻止近期仍活跃的批次；超过维护间隔且至少 10 分钟没有活动时必须重新到期，避免进程重启后的旧状态永久卡住自动维护。
...
子窗口实时图片 / 引用先把会话、方向、类型和占位正文写入 SQLite，再由 `core/wechat_ui_runtime.py` 在原 wxautox 回调线程调用原消息下载方法，并且必须先取得 owner FIFO 预约时段；发送、转发、素材读取、关系扫描、资料修改、新好友、监听和通讯录等任何当前动作都不可被抢占。下载后只用本地路径标量补充原事件并进入业务队列，原始 Message 不跨线程。`feature/listening.py` 和 `feature/message_routing.py` 不保留直接下载回退；全局监听必须使用 `UIClientFacade`，主动 quote / 素材操作由 owner 重新读取并唯一定位。
