...
机器人自己发出的消息由短时内存 `ReplyEchoTracker` 关联微信 `self` 回调，机器人回调不推进会话版本。每个气泡先按 delivery ID、会话类型、会话名、动作类型和内容登记为不可匹配的 `reserved`；UI owner 真正调用 handler 前转为 `active`，执行期间不受 TTL 限制，handler 返回或抛错后进入统一 60 秒 grace，明确在调用前取消则直接删除。callback 可能早于 `SendMsg()` 返回，因此 callback 确认和发送成功确认必须幂等地收敛到 SQLite 中同一个 outbound 事件；文本、图片和文件采用 callback 实际可见正文，语音保留 TTS 语义正文，结果不得依赖二者到达顺序。迟到回声可以把对应 `uncertain` 动作纠正为 `done`，但不能恢复已取消的后续气泡。不得恢复旧 echo 列表、补写 timer、history fallback 队列或停止时等待补写线程。
...
- `data/accounts/<wx_id>/message_store.sqlite3`：消息事实、聊天记录、会话版本、回复任务、逐气泡投递状态和非幂等 UI 提交围栏的唯一真源。
...
保存聊天历史并带入上下文时，AI 回复前只在启动首次回复、窗口恢复或当前入站不在本地尾部等强理由下读取一次当前窗口 `GetAllMessage()`，不周期巡检，也不滚动深历史。实机 wxautox4 的普通气泡 `Message.time` 为 `null`，时间只存在于独立 `TimeMessage`，因此快照规范化时让后续气泡继承最近的时间分隔，并保持同一时间组的窗口顺序。

当前可见窗口只有在能与本地可信历史找到明确锚点时，才允许补入缺少的对方消息和人工 self 消息；无锚点快照既不落盘，也不进入本轮模型上下文。实时回调可把 `message.id/hash/hash_text` 作为原始观察事实保存，但只有 native ID 用于强去重，hash 和正文不能删除两次真实出现的同文消息，也不作为跨重启身份。私聊按方向、类型、内容和邻近时间逐次匹配，群聊额外包含发送者；每条本地记录最多抵消一个可见气泡，因此重复短句和相同图片按出现次数保留。微信明确给出的时间分隔可用于邻近时间匹配；可见前缀或无时间快照为保持顺序而推算出的时间只决定插入位置，必须再按方向、类型和内容一对一抵消本地已有气泡，且该临时标记不得写入 history。日期解析同时接受微信快照的 `YYYY-MM-DD` 与本地 `YYYY/MM/DD`，对外历史统一渲染为后者。快照比较、补差额和写盘必须在同一把会话记忆锁内完成，重复执行同一快照新增数必须为 `0`。系统消息、`未播放` 和仅时长语音不写入模型上下文。冷却、锚点数量和读取上限均为内部参数，面板不暴露技术开关。
