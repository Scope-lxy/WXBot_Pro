...
- `tools/`：本地复现脚本、备份脚本和专项测试辅助，只放临时、测试性质工具；`probe_wxauto_voice_messages.py` 用于只读检查 wxautox4 当前窗口里的语音消息，传 `--to-text` 时会调用官方 `VoiceMessage.to_text()`，属于会操作微信 UI 的实机探针。
- `tests/`：行为保护测试。
...
- 微信 UI 锁保护的是一整轮真实微信动作，不只是主窗口切换。即使已经拿到缓存子窗口对象，`SendMsg`、`SendFiles`、`SendAudio`、群聊 `quote`、素材源历史读取、备注/标签编辑仍可能让微信焦点或可见列表变化，所以不能采用“先检查锁是否忙、稍后再发送/读取”的 check-then-act 写法。运行时任务发送、轻量延后发送队列、普通私聊 AI 文本/语音回复、私聊关键词回复、群聊回复、管理员回复、素材源读取和联系人资料编辑必须拿到全局 UI 锁后连续完成；私聊/运行时发送拿不到时进入轻量延后队列，通讯录自动维护、素材转发、关系扫描和上下文 UI 补洞这类任务拿不到锁时按各自策略跳过、稍后重试或延后，避免正在进行的一轮对话或 UI 任务被新动作打断。
...
- AI 回复发送前统一走回复预处理：先按 `clean_ai_reply_switch` 决定是否使用清洗结果，再用 `evaluate_reply_preprocess_admission()` 做准入；当前只拦截空回复、超过 `reply_preprocess_max_chars`、高确定性内部标记和短行首内部字段。不要从脏输出里提取“最终回复”，不合格时只允许重写一次。重写 prompt 固定为“只输出最终的回复消息，控制在 {最长字数} 字以内，不要解释，不要输出内部字段”。重写接口异常或返回 API 错误时走 `api_error_reply`；重写后仍不合格才走 `reply_preprocess_fallback_reply`，留空则静默。通过预处理后再进入拆分、文字 / 语音发送，发送成功后才标记一次性状态。
- 回复预处理日志使用中文冒号格式，并只记录规则结果，不记录完整 AI 回复。格式为 `回复预处理拦截，原因：超长，详情：字数 120/100，动作：准备重写一次`、`原因：命中内部字段，详情：命中字段：最终回复正文：` 或 `回复预处理重写后仍不合格，原因：空回复，详情：清洗后为空`。观察误伤时看原因和详情，不要临时把用户聊天全文写进日志。
- `api_error_reply`、回复预处理异常兜底和轮数超限固定回复的“只回复一次”状态都复用 `ReplyCountStore` 的回复循环窗口。新增同类提醒时，不要另起重置时间或独立状态文件。
...
- 身份校准跟随通讯录维护、手动建档和备注修复成功触发，不在每条消息热路径里刷新通讯录或扫描目录；校准状态内嵌在通讯录档案，不再维护独立身份索引真源。`identity_calibration.pending` 只保存 `fingerprint/status/reason/old_name/new_name` 等 v2 最小字段，不保存旧快照或完整身份表。
