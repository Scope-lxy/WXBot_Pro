...
`TimeMessage` 只作为可选的原生显示时间继承给后续气泡，不参与身份和去重；没有原生时间的补洞事件按窗口顺序放在当前批次之前，内部排序时间不对面板和模型显示。最近历史、删除边界、尾部计划和批量写入必须在同一个 `BEGIN IMMEDIATE` 事务中完成；无历史重合且清空记录后尚未形成新历史时不得补回旧气泡。补洞使用当前事件 ID 与窗口序号生成稳定批次身份，不推进会话版本、不创建回复任务；本次写入的消息由补洞结果直接追加到本轮原始 history，再统一执行模型可见过滤和条数裁剪，不能等到下一轮才生效。
