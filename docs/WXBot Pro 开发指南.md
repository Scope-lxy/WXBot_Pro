...
- 轻量延后期间同一好友已有新消息被处理、单次恢复未成功、或首次补窗失败但已进入延后重试时，日志使用 `INFO`；等待超过 600 秒、重建异常或最终放弃时使用 `WARNING`；恢复成功使用 `SUCCESS`。
...
- 私聊失败或最终未识别的语音按“没有有效输入”处理，静默忽略，不发送兜底提示。多条待识别语音里只要恢复出任意有效正文，成功识别的语音正常进入 AI，其它超时或失败语音直接丢弃。
...
- 运行日志等级按用户可处理程度和运营观察价值分层：普通消息处理、规则命中、排队、读取进度、会话记忆更新、允许的静默策略和已兜住的失败使用 `INFO`；产生明确运营效果或关键节点完成使用 `SUCCESS`，例如建档 / 通讯录自动维护当前批次成功、备注修复成功、微信标签同步有成功、关键词回复成功、定时消息 / 素材转发 / AI 自动转发 / 自定义转发成功、好友申请发送或通过成功、监听器恢复；任务实例或局部动作失败但机器人仍可继续运行使用 `WARNING`，例如定时消息 / 素材转发 / AI 自动转发失败、微信标签同步全失败、备注修复失败、回复 / 语音回复触发上限、API 错误或轮数超限提示发送失败；监听器、微信客户端、主线程或核心初始化受损时使用 `ERROR`。
