...
- UI 卡死恢复：wxautox4 的 UI 锁先在 30 秒超时并让当前动作收敛为失败或 `uncertain`；UI owner 还有 10 秒处理该异常，仍未恢复才由 watchdog 退出。运行中的机器人用专用退出码 `86`，`打开软件.bat` 在滚动 30 分钟最多 3 次的限制内恢复面板并自动启动机器人；用户停止阶段卡死用 `87`，同样恢复面板但保持机器人停止。
