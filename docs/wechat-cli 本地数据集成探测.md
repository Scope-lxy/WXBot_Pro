...
- 通讯录：WXBot 保存最多 `10000` 个好友；CLI 开启且账号校验通过时，底层会向 `wechat-cli contacts` 请求最多 `30000` 条原始行，用于过滤系统号、公众号后尽量拿满好友。CLI 基础资料会直接合并进通讯录档案，开启后按需检查，通讯录为空或距离上次 CLI 同步超过面板 `每轮维护间隔` 时自动执行；完整资料维护仍走微信界面自动维护。
