...
- 通讯录：WXBot 保存最多 `10000` 个好友；底层会向 `wechat-cli contacts` 请求最多 `30000` 条原始行，用于过滤系统号、公众号后尽量拿满好友。CLI 只维护基础身份索引，启动后按需检查，本地索引为空或距离上次 CLI 身份同步超过面板 `每轮维护间隔` 时自动执行；完整资料维护仍走微信界面自动维护。
- 私聊补洞：按 `memory_context_count` 读取最近 `60 ~ 210` 条本地历史，只把缺失记录补进 WXBot 现有聊天记录 JSON，不整套替换；补入前过滤未识别语音占位，并跳过 10 分钟内同方向、同类型、同内容的近时间重复。
- 群聊 CLI 补洞：先走 `wechat-cli sessions` 解析到 `@chatroom` 后再读历史；找不到群目标、重名群无法消歧或本地历史没有锚点时不猜测 CLI 目标。连续 CLI 最终失败后的低风险 UI 兜底、高风险滚动读取由聊天记录页的补洞开关统一控制。
...
- 通讯录已经按“CLI 基础身份索引 + UI 完整资料维护”接入：手动建档优先提取 CLI 可读联系人，自动 CLI 同步独立按 X 天维护昵称、备注、wxid；UI 自动维护继续负责地区、来源、标签等完整资料。
