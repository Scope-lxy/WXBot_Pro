...
- 上下文补洞：聊天记录页 `自动补最近上下文` 开启时，私聊和群聊都会优先用 CLI 读取本地历史；读取条数按 `memory_context_count` 归一到最近 `60 ~ 210` 条，只把缺失记录补进 WXBot 现有聊天记录 JSON，不整套替换。补入前过滤未识别语音占位，并跳过 10 分钟内同方向、同类型、同内容的近时间重复。
