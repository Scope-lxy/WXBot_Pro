...
2026-08-07 在当前微信和 `wxautox4==41.1.1.post1` 上，对 15 个已打开的监听子窗口（14 个私聊、1 个群聊）完成了“打开 -> 直接重建 -> 再打开 -> 关闭”实测：`StopListening()` 在直接重建时已关闭全部旧子窗口，重建后对旧窗口的关闭请求均为安全空操作，新窗口也都能正常关闭。因此重建不需要、也不应增加“先关闭所有旧窗口”的独立阶段；全局扫描模式的动态窗口记录仍须保留，供首次扫空后的按需补窗使用。
