...
- 当前 AI 轮次的“消息时间”在全局新消息入站时确定：优先取当前批次最后一条真实入站消息之前最近的 `TimeMessage`；批次没有时间条时，只从该消息控件向前检查最多 30 个同级控件，并用 wxautox4 最终确认时间条。结果只写入当前批次最后一条真实入站消息的 `native_time`；AI 只读当前事件自己的时间，不从补洞或历史记录继承。查不到或查询失败时按无时间继续，不能阻断回复。
