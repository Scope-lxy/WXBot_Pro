...
- 微信 UI 锁保护一整轮真实微信动作；缓存子窗口的 `SendMsg` / `SendFiles` / `SendAudio`、群聊 `quote`、素材源历史读取、备注/标签编辑也算会碰 UI。不要只做“检查锁是否忙”后再发送；运行时发送、轻量发送队列、私聊/群聊自动回复、管理员回复和素材读取都必须拿到锁后连续完成，拿不到就等待、排队或延后。
