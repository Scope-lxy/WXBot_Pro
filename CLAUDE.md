...
- `wechat-cli` 是 `venv/tools/wechat-cli/` 下的本地只读高速源，只能在账号绑定/活体校验通过后用于通讯录、私聊补洞和关系扫描；同名补洞必须满足 4 条锚点消歧，不能猜 wxid 或把 CLI 额外字段塞进现有 JSON；补洞写入前要过滤未识别语音占位，并按同方向/类型/内容 10 分钟近重复去重。
