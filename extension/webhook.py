...
CONFIG_PATH = os.path.join(DATA_DIR, "config", "webhook.json")
...
def send_webhook(title: str, content: str, config: Optional[Dict[str, Any]] = None) -> Tuple[bool, str]:
    ...
    if content_type.lower().startswith("application/json"):
        ...
        kwargs["data"] = _render(cfg.get("body", ""), title, content)
        ...
        if 200 <= response.status_code < 300:
            ...
            if isinstance(response_json, dict):
                ...
                if code not in (None, 0):
                    msg = response_json.get("msg") or response_json.get("message") or response_text[:200]
                    return False, f"Webhook provider rejected message: code={code}, msg={msg}"
                if status_code not in (None, 0):
                    msg = response_json.get("StatusMessage") or response_text[:200]
                    return False, f"Webhook provider rejected message: StatusCode={status_code}, msg={msg}"
